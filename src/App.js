...
function App() {
  ...
  const renderStories = stories.map(st => <li key={st.id} onClick={() => handleStorySelect(st.id)}><FaveButton /> <strong>{st.headline}</strong> {st.snippet}</li>)
  ...
  return(
    <div className="news-reader">
      ...
      <aside>
        ...
        <h3 id="greeting">Hi there, {username ? username : 'friend'}!</h3>

        {/* Form handling */}
        <form onSubmit={handleFormSubmit}>
          <input type="text" id="nameInput" name="nameInput" placeholder="That's not my name!" value={nameInput} onChange={handleInput}/>
